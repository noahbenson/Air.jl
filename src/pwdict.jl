    ...
    return esc(
        quote
            ...
            function $name{K,V,W}() where {K,V,W<:Number}
                return $name{K,V,W}(PWDictHeap{K,W,$dicttype{K,Int}}(>), $dicttype{K,V}())
            end
            ...
            function $name{K,V}(::Tuple{}) where {K,V}
                return $name{K,V,Float64}(
                    PWDictHeap{K,Float64,$dicttype{K,Int}}(>), $dicttype{K,V}()
                )
            end
            ...
            function $name()
                return $name{Any,Any,Float64}(
                    PWDictHeap{Any,Float64,$dicttype{Any,Int}}(), $dicttype{Any,Any}()
                )
            end
            ...
            Air.setindex(u::$name{K,V,W}, v, k::J) where {K,V,W,J} = begin
                (get(u.heap._index, k, 0) == 0) && throw(
                    ArgumentError(
                        "when inserting into PWDict, value and weight are required"
                    ),
                )
            ...
            Air.setindex(
                u::$name{K,V,W}, v::Tuple{VV,WW}, k::J
            ) where {K,V,W,VV,WW<:Number,J} = begin
                return push(u, k => v)
            end
            Air.setindex(
                u::$name{K,V,W}, v::Pair{VV,WW}, k::J
            ) where {K,V,W,VV,WW<:Number,J} = begin
                return push(u, k => v)
            end
...
function Base.empty(
    s::PWDict{K,V,W}, ::Type{J}=K, ::Type{U}=V, ::Type{X}=W
) where {K,V,W,J,U,X}
    return PWDict{J,U,X}()
end
function Base.empty(
    s::PWIdDict{K,V,W}, ::Type{J}=K, ::Type{U}=V, ::Type{X}=W
) where {K,V,W,J,U,X}
...
function Base.hash(u::PWD) where {K,V,W,PWD<:AbstractPWDict{K,V,W}}
    h = length(u)
    for (k, v) in u.dict
