...
macro _plindict_code(name::Symbol, eq, h)
    return esc(
        quote
            ...
            # Base methods.
            Base.empty(s::$name{K,V}, ::Type{J}=K, ::Type{U}=V) where {K,V,J,U} =
                $name{J,U}()
            ...
            @inline Base.iterate(u::$name{K,V}, ii::Int) where {K,V} = begin
                ...
                return (
                    Pair{K,V}(ks[ii], vs[ii]), #(@inbounds ks[ii], @inbounds vs[kk]),
                    ii + 1,
                )
            end
...
macro _pdict_code(name::Symbol, eq, h, lindict)
    return esc(
        quote
            ...
            Base.in(x::Pair, u::$name{K,V}) where {K,V} = in(x, u, (==))
            # And the Air methods.
            Air.push(u::$name{K,V}, x::Pair) where {K,V} = setindex(u, x.second, x.first)
